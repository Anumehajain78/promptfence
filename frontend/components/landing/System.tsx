...
const LAYERS: Layer[] = [
  { name: "AI agent", kicker: "Strands · Bedrock", lines: [["agent", "support-agent"], ["intent", "refund"], ["amount", inr(9000)], ["customer", "#4474"]] },
  { name: "PromptFence", kicker: "Intercept", lines: [["event", "Request received"], ["request", "#40"], ["ts", "14:02:31.114"], ["tool call", "Held"]] },
  {
    name: "Cedar",
    kicker: "Policy",
  ...
  { name: "Decision", kicker: "Output", lines: [["decision", "Deny", "deny"], ["reason", "Cumulative session limit exceeded"], ["approval path", "not applicable"]] },
  { name: "Real tool", kicker: "refund()", lines: [["execution", "Not called", "deny"], ["side effects", "none"], ["attack", "stopped at the fence"]] },
];
...
function LayerRow({ layer, index, on, onEnter }: { layer: Layer; index: number; on: boolean; onEnter: (i: number) => void }) {
  ...
  return (
    ...
    >
      ...
      />
      <div>
        <div className="text-[13px] text-grey-500">{layer.kicker}</div>
        <div
...
export function System() {
  ...
  return (
    <section id="system" aria-label="The system" className="scroll-mt-24 md:scroll-mt-16">
      <div className={`mx-auto max-w-[1440px] ${PAGE_X} ${SECTION_Y}`}>
        <Reveal>
          <Kicker aside={<span>Following request <span className="font-mono">#40</span></span>}>
            The system
          </Kicker>
        ...
        <div className="mt-[clamp(48px,6vw,96px)] border-t border-ink">
          {LAYERS.map((layer, i) => (
            <LayerRow key={layer.name} layer={layer} index={i} on={i <= reached} onEnter={onEnter} />
          ))}
        </div>
      </div>
    </section>
